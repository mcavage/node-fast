...
function Client(options) {
    ...
    function onConnect(connectErr, conn) {
            ...
            conn.once('close', function (had_err) {
                ...
                if (self.reconnect) {
                    ...
                    self.timer = setTimeout(function () {
                        self._pendingConn = connect(self,
                                                    options,
                                                    onConnect);
        ...
        self.conn = conn;
        self.conn.setKeepAlive(true, 60000);
        ...
        self.messageDecoder.on('message', function onResponse(msg) {
            if (!msg.data || !msg.data.m || !msg.data.d) {
                self.emit('error', new Error('bad message'));
                return;
    ...
    this._pendingConn = connect(this, options, onConnect);
}
...
Client.prototype.close = function close() {
    ...
    if (this._pendingConn) {
        this._pendingConn = this._pendingConn.abort();
